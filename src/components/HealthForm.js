...
import ProgramResponse from "@/components/ProgramResponse";
...
const HealthForm = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const message = `
      Génère une réponse JSON strictement structurée avec :
      ...
      FORMAT JSON REQUIS :
      {
        "personalInfo": {
          ...
          "physicalProfile": {
            "height": ${data.taille},
            "currentWeight": ${data.poidsActuel},
            "targetWeight": ${data.poidsObjectif}
          }
        },
        "trainingProfile": {
          "fitnessLevel": "${data.niveauForme}",
          "trainingFrequency": "${data.frequenceEntrainement}",
          "preferredLocation": "${data.lieuEntrainement}",
          "sessionDuration": "${data.dureeSouhaitee}"
        },
        "mainObjectives": ${JSON.stringify(data.objectifs)},
        "additionalInfo": {
          "medicalConstraints": "${data.contraintesMedicales || "None"}",
          "availableEquipment": "${data.equipementsDisponibles || "None"}",
          "preferredSports": "${data.sportsPreferences || "None"}",
          "motivationLevel": ${data.niveauMotivation},
          "stressLevel": ${data.niveauStress}
        },
        "recommendedProgram": {
          "trainingPlan": [],
      ...
      Tâches :
      1. Remplir chaque section du JSON
      2. Personnaliser le contenu
      3. Fournir un programme détaillé
      4. Donner des recommandations adaptées
      `;
      ...
      console.log("Réponse du coach:", result.response);
      const jsonResponse = result.response.replace(/```json\n|```/g, "").trim();
      setProgramResponse(jsonResponse);
      setShowForm(false);
    ...
    <div className="w-full max-w-3xl mx-auto p-4">
      {showForm ? (
        ...
        <>
          {programResponse && (
            <div className="space-y-4">
              <ProgramResponse response={programResponse} />
